    ...
    try:
        ...
        await update.message.reply_document(
            ...
            caption=f"✅ SMALI EXTRACTED\n\n"
                    ...
                    f"✏️ Edit karo → wapas upload karo → recompile + sign ho jaayega\n\n"
                    f"⚡ VTX DEX | {DEV_NAME}"
        )
        
        # Save state for recompile
...
if action == 'dump':
    ...
    await processing_msg.edit_text("❌ Use a command first: /dump, /armkiller, /smali, /anti-debug, /anti-screenshot, /root-detect")
    os.remove(file_path)
...
app.add_handler(CommandHandler("anti-debug", anti_debug_cmd))
app.add_handler(CommandHandler("anti-screenshot", anti_screenshot_cmd))
app.add_handler(CommandHandler("root-detect", root_detect_cmd))
