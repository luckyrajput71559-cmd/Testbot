...
def check_access(user_id: int) -> Tuple[bool, str]:
    ...
    key = user[3]  # key_value
    if not key:
        return False, "🔑 No active key. Use /redeem"
    
    # ★ PANEL SE RE-VERIFY KARO ★
    try:
        ...
        if panel_resp.status_code == 200:
            ...
            if panel_data.get('status') != True:
                # Panel se key expire ho gayi
                reason = panel_data.get('reason', 'Key expired')
                return False, f"❌ {reason}"
        else:
            # Panel error — phir bhi local expiry check karo
            pass
    except:
        # Panel connection fail — local expiry check karo
        pass
