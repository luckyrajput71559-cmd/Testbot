...
def check_access(user_id: int) -> Tuple[bool, str]:
    ...
    try:
        ...
        if panel_resp.status_code == 200:
            ...
            # Agar panel se status false aaye — matlab key expire/invalid
            ...
            if panel_days:
                try:
                    expiry_days = int(float(panel_days))
        ...
        if panel_resp.status_code == 200:
            ...
            if panel_data.get('status') == True:
                ...
                if panel_days is not None:
                    ...
                    if panel_expiry:
                        try:
                            ...
                            expiry_days = max(0, (exp_dt - now_ist()).days)
                        except:
                            expiry_days = 1
                            expiry = (now_ist() + timedelta(days=1)).isoformat()
                    else:
                ...
                if expiry_days == 0:
                    ...
                    display_days = f"{expiry_days} days"
