...
def redeem_key(user_id: int, key: str) -> Tuple[bool, str]:
    ...
    try:
        panel_resp = requests.post(
            PANEL_URL,
            data={"game": "VTX", "user_key": key, "serial": str(user_id)},
            timeout=10
        )
...
public function index()
{
    ...
    $builder->where('key_value', $user_key);
    $builder->where('is_active', 1);
    $query = $builder->get();
    $keyData = $query->getRow();
    
    if (!$keyData) {
        return $this->response->setJSON(['status' => false, 'reason' => 'Invalid key']);
    }
    
    if (strtotime($keyData->expires_at) < time()) {
        return $this->response->setJSON(['status' => false, 'reason' => 'Key expired']);
    }
    ...
    return $this->response->setJSON([
        ...
        'data' => [
            'type' => $keyData->type,
            'days' => (strtotime($keyData->expires_at) - time()) / 86400,
            'devices' => $keyData->max_devices,
