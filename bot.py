...
def sign_apk(apk_path: str) -> str:
    """APK ko V1 + V2 + V3 se sign karo — automatic"""
    tool_dir = os.path.abspath(TOOLS_DIR)
    keystore = os.path.join(tool_dir, "testkey.keystore")
    alias = "test"
    storepass = "test"
    ...
    # Fallback — jarsigner (sirf V1)
    cmd = f"jarsigner -keystore {keystore} -storepass {storepass} -keypass {keypass} {apk_path} {alias}"
    subprocess.run(cmd, shell=True)
    
    # Zipalign (agar available ho)
    subprocess.run(f"zipalign -v 4 {apk_path} {apk_path}.aligned", shell=True)
    if os.path.exists(f"{apk_path}.aligned"):
        os.replace(f"{apk_path}.aligned", apk_path)
    
    return "✅ Signed with V1 (jarsigner fallback)"
...
async def process_add_root_detect(update: Update, context: ContextTypes.DEFAULT_TYPE, file_path: str, processing_msg):
    """Root detection ADD process"""
    user_id = update.effective_user.id
    ...
    try:
        ...
        await processing_msg.edit_text("🛠️ Adding root detection...")
        
        # Root detection smali file banao
        root_smali_dir = os.path.join(dec_dir, "smali", "com", "vtx", "dex")
        os.makedirs(root_smali_dir, exist_ok=True)
        
        root_smali = """.class public Lcom/vtx/dex/RootDetect;
.super Ljava/lang/Object;
...
"""
        ...
        with open(os.path.join(root_smali_dir, "RootDetect.smali"), "w") as f:
            f.write(root_smali)
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == 'dump':
        ...
        await processing_msg.edit_text("❌ Use a command first: /dump, /armkiller, /smali, /anti_debug, /anti_screenshot, /root_detect, /recompile, /sign, /add_root_detect, /add_screenshot_block, /add_anti_debug")
        os.remove(file_path)
